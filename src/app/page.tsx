...
import { Share2, PanelLeftClose, PanelLeftOpen, MousePointer2, Music2, Guitar, Sparkles } from 'lucide-react';
import { useState } from 'react';
import { StringGroupAnalysis } from '@/types/music';
import { clsx } from "clsx";

type AppMode = 'analysis' | 'interactive' | 'chordscale' | 'composer';
...
export default function Home() {
  ...
  const { clearGenerated: clearProgressions } = useProgressionStore();
  ...
  const handleModeChange = (mode: AppMode) => {
    ...
    // Clear composer state when leaving composer mode
    if (currentMode === 'composer' && mode !== 'composer') {
      clearProgressions();
    }
  };
  ...
  return (
    <main className="min-h-screen bg-slate-950 text-slate-200 p-4 md:p-8 font-sans selection:bg-indigo-500/30">
      ...
      {/* Composer Mode - Full Width Layout */}
      {currentMode === 'composer' ? (
